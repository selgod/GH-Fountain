...
public class PlayerViewController implements Initializable {
	...
	public void selectMusic() {
		...
		// if
		// (mediaPlayer.statusProperty().getValue().compareTo(Status.PLAYING)!=0){
		// mediaPlayer.dispose();
		// }
		// if
		// (mediaPlayer.statusProperty().getValue().compareTo(Status.PAUSED)!=0){
		// mediaPlayer.dispose();
		// }
		// if
		// (mediaPlayer.statusProperty().getValue().compareTo(Status.STOPPED)!=0){
		// mediaPlayer.dispose();
		// }
		// try{
		// mediaPlayer.dispose();
	...
	public void createTimeline(double tenths) {
		...
		for (int i = 0; i < intTenths + 1; i++) {
			...
			if (i < 17) { // because the array is not square this needs to be
							// here
	...
	public void playSong() {
		try {
			...
			// String source = new
			// File(music2.getDirectoryFile()).toURI().toString();
			// Media media = new Media(source);
			// mediaPlayer = new MediaPlayer(media);
			// mediaPlayer.setVolume(volume.getValue());
			// songName.setText(music2.getName());
