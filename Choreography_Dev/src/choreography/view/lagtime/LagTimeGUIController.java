...
public class LagTimeGUIController implements Initializable {
	...
	public void initialize(URL url, ResourceBundle rb) {
		...
		lagTimeTable.setEditable(true);
		delayName.setEditable(true);
		delayTime.setEditable(true);

		Callback<TableColumn<LagTime, String>, TableCell<LagTime, String>> cellFactory = new Callback<TableColumn<LagTime, String>, TableCell<LagTime, String>>() {
		...
		delayName.setCellValueFactory(new PropertyValueFactory<LagTime, String>("delayName"));
		delayName.setCellFactory(cellFactory);
		delayName.setOnEditCommit(new EventHandler<CellEditEvent<LagTime, String>>() {
			@Override
			public void handle(CellEditEvent<LagTime, String> t) {
				((LagTime) t.getTableView().getItems().get(t.getTablePosition().getRow())).setDelayName(t.getNewValue());
			}
		});

		delayTime.setCellValueFactory(new PropertyValueFactory<LagTime, Double>("delayTime"));
		;

		Callback<TableColumn<LagTime, Double>, TableCell<LagTime, Double>> doubleCellFactory = new Callback<TableColumn<LagTime, Double>, TableCell<LagTime, Double>>() {
			@SuppressWarnings("rawtypes")
			@Override
			public TableCell call(TableColumn p) {
		...
		delayTime.setOnEditCommit(new EventHandler<CellEditEvent<LagTime, Double>>() {
			...
			public void handle(CellEditEvent<LagTime, Double> t) {
				((LagTime) t.getTableView().getItems().get(t.getTablePosition().getRow())).setDelayTime(t.getNewValue());
			}
		});
		...
		addButton.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent e) {
				lagTimeTable.getItems().add(new LagTime(addDelayName.getText(), Double.parseDouble(addDelayTime.getText())));
				addDelayName.clear();
				addDelayTime.clear();
			}
		});
