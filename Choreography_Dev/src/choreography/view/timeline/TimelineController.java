...
public class TimelineController implements Initializable {
	...
	/**
	 * Just fills in the lightRecArray with gray,
	 * it doesn't actually delete from the timeline.
	 ...
	 * 
	 * todo
	 * Only operates if first==true, why?
	 * this should push to the timeline, then pull from timeline when repainting
	 */
	public void delete(int timeIndex, int channelIndex, int length, boolean first){
		if(first){
			...
			lightRecArray[timeIndex][channelIndex].setFill(Color.LIGHTGRAY);
			timeline.setLightFcw(off, timeIndex, timeIndex+length);//not sure if length is needed?
		}
	...
	/**
	 * Sets channelAddresses and labelNames to match what is in timeline.getChannelColormap().keySet()
	 */
	...
	/**
	 * Adds constraints and words to the labels on the side of the light
	 * grid timeline. Also adds listeners to the labelScrollPane and 
	 * TimelineScrollPane
	 */
	...
	/**
	 * Checks to see if query is already in channelAddresses
