...
public class ChoreographyController implements Initializable {
	...
	public void initialize(URL url, ResourceBundle rb) {
		...
		quitMenuItem.setOnAction(new EventHandler<ActionEvent>() {
			@Override
			public void handle(ActionEvent t) {
				Action result = Dialogs.create().title("Quit?").masthead("").message("Are you sure you want to quit?")
						.showConfirm();
				if (result != Actions.YES) {
					...
					if (isSaved) {
						...
						Action saveResult = Dialogs.create().title("Save?")
								...
								.message("Would you like to save before quiting?").showConfirm();
	...
	public void openMapFileMenuItemHandler() {
		try {
			...
			Dialogs.create().title("Invalid MAP file")
					.message("You've selected an invalid MAP file. " + "Please try again.").showError();
			Logger.getLogger(ChoreographyController.class.getName()).log(Level.SEVERE, null, ex);
		}
	}
		...
		Dialogs.create()
				...
				.message(
						"The Grand Valley State University senior project team, Excalibur Solutions, created the GHMF Choreography Studio on April 15, 2014. "
								...
								+ "Musical Fountain located in Grand Haven Michigan.  ").masthead("About")
				.showInformation();
	}
